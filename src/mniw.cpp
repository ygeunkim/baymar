...
//' @noRd
// [[Rcpp::export]]
Rcpp::List forecast_bmar_mniw(int num_chains, int lag, int step, Eigen::MatrixXd response_mat,
													 	 	Rcpp::List fit_record, Eigen::VectorXi seed_chain, int nthreads) {
	auto forecaster = std::make_unique<baymar::MatMniwForecastRun>(num_chains, lag, step, response_mat, fit_record, seed_chain, nthreads);
